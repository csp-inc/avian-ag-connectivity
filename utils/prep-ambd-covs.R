#-----------------------------------------------------------
# PREP AMERICAN BLACK DUCK COVARIATE RASTERS FOR ANALYSIS
#-----------------------------------------------------------
...
tifs <- tifs[str_detect(tifs, "ambd")]
...
m_names <- c("Ag_mean","Urban_mean","Transport_mean","Energy_mean","elevation_mean",
             "slope_mean","aspect_mean","vrm_1km_mean","swe_mean","tmin_mean",
             "tmax_mean","prcp_mean")
sd_names <- c("Ag_stdDev","Urban_stdDev","Transport_stdDev","Energy_stdDev","elevation_stdDev",
             "slope_stdDev","aspect_stdDev","vrm_1km_stdDev","swe_stdDev","tmin_stdDev",
             "tmax_stdDev","prcp_stdDev")
p_names <- c("water_pcov","wet_all_pcov","wet_herb_pcov","tidal_pcov")
...
rbuff <- st_read(dsn = 'data/focal-species-range-maps/black-duck-iucn-range',
                 layer = 'ambd_buff_250km')
...
pred_covs <- cbind(pred_cov_temp, pred_xy) %>% 
  drop_na()
save(pred_covs, file = 'data/ebird/ambd-pred-covs.rda')

# Get checklist data
bird = read.csv('data/ebird/ebd_ambd_2014_2018.csv', header = T) %>% 
...
save(bird_out, file = "data/ebird/ebd_ambd_2014_2018_wcovs.rda")
# write_csv(bird_out, file = "data/ebird/ebd_ambd_2014_2018_wcovs.csv")
