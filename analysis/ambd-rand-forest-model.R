#####################################################################
# FIT SPATIALLY SUB-SAMPLED RANDOM FOREST MODEL FOR BLACK DUCK
...
# SET MODEL TYPE
# 'no-clim' excludes temperature covariates (stongly confounded with latitude for ambd)
mod_type = 'no-clim' # one of 'full' or 'no-clim'

# GET DATA AND FUNCTIONS
# Get species range map for creating sampling grid
rmap <- st_read(dsn = 'data/focal-species-range-maps/black-duck-iucn-range',
                layer = 'ambd_union_range') %>% 
  st_transform(5070)

# Read in species data and transform to appropriate crs
load('data/ebird/ebd_ambd_2014_2018_wcovs.rda', verbose = T)
bird <- bird_out%>% 
...
bird <- bird %>% 
  filter(day_of_year %in% c(78:180, 279:353))
...
if(mod_type=='full') {
  brd_split <- brd_ss %>% 
    select(species_observed,year,day_of_year,Ag_mean,Energy_mean,Transport_mean,Urban_mean,
           ...
           prcp_mean, water_pcov, wet_herb_pcov, wet_all_pcov, tidal_pcov,
...
fout = paste0('output/ambd-random-forest-', mod_type, '-', d, '.rda')
