#-----------------------------------------------------------
# PREP AMERICAN BLACK DUCK HABITAT SUITABILITY SURFACE
...
inFile = paste0('output/ambd-random-forest-', mod_type,'-20220403.rda')
load(file = inFile, verbose = TRUE)
# Read in prediction dataset
load(file = 'data/ebird/ambd-pred-covs.rda', verbose = TRUE)
...
pred_er <- bind_cols(pred_covs_eff, encounter_rate = pred_rf_cal) %>% 
  dplyr::select(latitude, longitude, encounter_rate) %>% 
...
# save the raster
outFile = paste0('output/ambd-hab-suitability-', mod_type, '.tif')
raster::writeRaster(pred_rast, outFile, overwrite = TRUE)
